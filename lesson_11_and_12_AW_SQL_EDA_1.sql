...
INNER JOIN databricks_ml.default.dim_product_subcategory         s
ON (p.ProductSubcategoryKey = s.ProductSubcategoryKey)
INNER JOIN databricks_ml.default.dim_product_category            c
...
INNER JOIN default.dim_product_subcategory         s
ON (p.ProductSubcategoryKey = s.ProductSubcategoryKey)
INNER JOIN default.dim_product_category           c
ON (s.ProductCategoryKey = c.ProductCategoryKey)
WHERE p.Status = 'Current' OR p.Status = 'NULL';
...
SHOW CREATE TABLE databricks_ml.default.dim_customer
...
FROM databricks_ml.default.dim_customer ;
...
SELECT salesterritorykey,
       ...
       ELSE 'NA' 
       END as CountryCD
from databricks_ml.default.dim_sales_territory t
)
